...
{% macro render_cpp_default_value(member) -%}
    ...
    {%- elif member.type.category in ["enum", "bitmask"] and member.default_value != None -%}
        {{" "}}= {{ get_namespace().lower() }}::{{as_cppType(member.type.name)}}::{{as_cppEnum(Name(member.default_value))}}
...
{%- endmacro %}
    ...
    struct ChainedStruct {
        ...
        {{ get_namespace().lower() }}::SType sType = {{ get_namespace().lower() }}::SType::Invalid;
    };
        ...
        {% if type.chained %}
            struct {{as_cppType(type.name)}} : ChainedStruct {
                {{as_cppType(type.name)}}() {
                    sType = {{ get_namespace().lower() }}::SType::{{type.name.CamelCase()}};
                }
        {% else %}
