...
TEST_F(AddTests, AddConstantAndInput) {
    ...
    const webnn::MLOperand c = builder.Add(a, b);
    ...
    const webnn::MLInput input = {dataA.data(), dataA.size() * sizeof(float)};
...
TEST_F(AddTests, AddTwoInputs) {
    const webnn::MLGraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::MLOperand a = utils::BuildInput(builder, "a", {3, 4, 5});
    const webnn::MLOperand b = utils::BuildInput(builder, "b", {3, 4, 5});
    const webnn::MLOperand c = builder.Add(a, b);
    const webnn::MLGraph graph = utils::AwaitBuild(builder, {{"c", c}});
    const std::vector<float> dataA = {
        0.08939514,  -1.5887482,  0.8545348,   0.20523034, -0.41728342, 1.01752,     0.19677015,
        ...
        -0.55656946, -0.735903,   0.22050636,  -0.5008282, -1.3132697,  1.6642882,   -0.48397836,
        0.20099205,  -0.28786168, 1.3315053,   -0.41619393};
    const webnn::MLInput inputA = {dataA.data(), dataA.size() * sizeof(float)};
    const std::vector<float> dataB = {
        ...
        0.3222213,   1.0590587,   -1.7948701,  -1.7195907,  -0.9120889,  -0.9391962,  -0.2566791,
        -0.5464537,  1.4351872,   0.5705938,   -0.30327085};
    const webnn::MLInput inputB = {dataB.data(), dataB.size() * sizeof(float)};
...
TEST_F(AddTests, AddBroadcast) {
    const webnn::MLGraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::MLOperand a = utils::BuildInput(builder, "a", {3, 4, 5});
    const webnn::MLOperand b = utils::BuildInput(builder, "b", {5});
    const webnn::MLOperand c = builder.Add(a, b);
    const webnn::MLGraph graph = utils::AwaitBuild(builder, {{"c", c}});
    const std::vector<float> dataA = {
    ...
    const std::vector<float> expectedValue(
        {0.5484205,   1.7485408,   -2.6178582,  -0.7418642,  0.32369673,  2.123247,    1.7987677,
         ...
         0.5918797,   -3.3222437,  -3.1333683,  0.6276101,   -0.63919926, 3.1944916,   -0.8484118,
         -1.8507047,  0.99363536,  -0.15031523, 1.3891755,   0.54554,     -0.59508437, 1.2128129,
         -0.60254204, 1.123076,    -1.3494825,  -0.17931074, 0.26426458,  1.8173906,   3.145669,
         -1.0727432,  -2.354507,   0.39223427,  1.5153855,   1.3189576,   -0.8068688,  -2.2351713,
