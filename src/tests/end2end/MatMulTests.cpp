...
TEST_F(MatMulTests, MatMul1d) {
    ...
    const webnn::Operand a = utils::BuildInput(builder, "a", {4});
    ...
    const webnn::Operand c = builder.Matmul(a, b);
    ...
    const webnn::Input input = {aData.data(), aData.size() * sizeof(float)};
...
TEST_F(MatMulTests, MatMul3d) {
    const webnn::GraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::Operand a = utils::BuildInput(builder, "a", {2, 3, 4});
...
TEST_F(MatMulTests, MatMul3dx2dGet3d) {
    const webnn::GraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::Operand a = utils::BuildInput(builder, "a", {1, 3, 4});
...
TEST_F(MatMulTests, MatMul4d) {
    const webnn::GraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::Operand a = utils::BuildInput(builder, "a", {1, 2, 3, 4});
    const std::vector<float> bData = {
