...
TEST_F(MatMulTests, MatMul1d) {
    ...
    const webnn::MLOperand a = utils::BuildInput(builder, "a", {4});
    const std::vector<float> bData = {0.8782074, 0.22533207, 0.7134056, 0.04190519};
    const webnn::MLOperand b =
        utils::BuildConstant(builder, {4}, bData.data(), bData.size() * sizeof(float));
    const webnn::MLOperand c = builder.Matmul(a, b);
    const webnn::MLGraph graph = utils::AwaitBuild(builder, {{"c", c}});
    const std::vector<float> aData = {0.9025404, 0.89538723, 0.16789329, 0.7440875};
    const webnn::MLInput input = {aData.data(), aData.size() * sizeof(float)};
    const webnn::MLResult result = utils::AwaitCompute(graph, {{"a", input}}).Get("c");
...
TEST_F(MatMulTests, MatMul2dx1d) {
    const webnn::MLGraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::MLOperand a = utils::BuildInput(builder, "a", {3, 4});
...
TEST_F(MatMulTests, MatMul3d) {
    const webnn::MLGraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::MLOperand a = utils::BuildInput(builder, "a", {2, 3, 4});
...
TEST_F(MatMulTests, MatMul3dx2dGet3d) {
    const webnn::MLGraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::MLOperand a = utils::BuildInput(builder, "a", {1, 3, 4});
...
TEST_F(MatMulTests, MatMul4d) {
    const webnn::MLGraphBuilder builder = webnn::CreateGraphBuilder(GetContext());
    const webnn::MLOperand a = utils::BuildInput(builder, "a", {1, 2, 3, 4});
    const std::vector<float> bData = {
